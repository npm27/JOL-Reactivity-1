...
#Drop unused columns
JOL1 = JOL[ , -c(2:6, 8, 10:12)] #Will use this one for IOC
JOL2 = JOL[ , -c(2:6, 8:12)] #Will use this one for reactivity

JOL2$Task = rep("JOL")

Study = Study[ , -c(2:6, 8:11)]
...
jols.ratings = cast(JOL1[ , -4], Username ~ Direction, mean, na.rm = T)
jols.ratings2 = scale(jols.ratings) #No outliers

#JOL group recall by direction
jols = cast(JOL1, Username ~ Direction, mean, na.rm = T)
jols2 = scale(jols) #Close but no outliers

#Study group recall by direction
study = cast(Study[ , -4], Username ~ Direction, mean, na.rm = T)
...
##Run the t-tests
temp = t.test(JOL.ph$S, JOL.ph$B, paired = F, p.adjust.methods = "bonferroni")
temp
...
#B vs S
temp = t.test(reactivity2$B, reactivity2$S, paired = F, p.adjust.methods = "bonferroni")
temp
round(temp$p.value, 3)
temp$statistic
(temp$conf.int[2] - temp$conf.int[1]) / 3.92 #Sig
